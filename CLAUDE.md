...
**Status saat ini** selalu dibaca dari [`IMPLEMENTATION-STATUS.md`](docs/IMPLEMENTATION/IMPLEMENTATION-STATUS.md); jangan menyalin status phase atau PR ke berkas ini. Rencana total tetap 9 phase · 167 PR — [`docs/IMPLEMENTATION/README.md`](docs/IMPLEMENTATION/README.md).
...
3b. **Muat skill sesuai pohon yang disentuh** sebelum menulis kode: `frontend-ui-engineering` untuk `apps/web` dan `apps/mobile`; `api-and-interface-design` untuk `apps/api` (kontrak endpoint, batas modul). Skill melengkapi pembacaan docs di atas, tidak menggantikannya; bila keduanya bertentangan, docs yang berlaku. PR yang menyentuh dua pohon memuat keduanya.
