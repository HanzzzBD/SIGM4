...
| AS-15 | Tersedia akun Firebase untuk push notification dan akun **Google Gemini Developer API berbayar (*paid tier*)** untuk chatbot, beserta anggaran penggunaannya. Tier gratis tidak memenuhi DP-AI-04 dan tidak boleh dipakai untuk data SIGM4. |
...
| RS-21 | Gemini Developer API tidak menjamin residensi data; prompt dan hasil tool chatbot dapat diproses atau di-*cache* di yurisdiksi mana pun tempat penyedia memiliki fasilitas | Kepatuhan | Sedang | Tinggi | Paid tier (tanpa pemakaian untuk pelatihan model) + DPA penyedia; konteks yang dikirim sudah dibatasi role & scope tanpa identitas pribadi (DP-AI-01) dan disaring *field allow-list* (DP-AI-02); persetujuan tertulis sekolah atas pemrosesan lintas yurisdiksi menjadi prasyarat GL-07 dan tercatat sebagai `TBD-AI-D`. Bila persetujuan tidak diberikan, jalur *scope-cut* adalah menonaktifkan chatbot lewat `AI-CTL-09` |
