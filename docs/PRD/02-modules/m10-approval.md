...
- **A4 — Approver adalah pemohon itu sendiri:** Sistem otomatis melewati langkah tersebut dan mencatat `dilewati — konflik kepentingan` (BR-039). Bila **seluruh** langkah terlewati karena hal ini, pengajuan diarahkan ke *fallback approver* opsional pada aturan; bila aturan tidak menetapkannya, berlaku role Administrator (RE-11). Langkah yang approver-nya nonaktif diperlakukan sama — dilewati, lalu jatuh ke jalur yang sama (RE-13).
