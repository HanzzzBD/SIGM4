...
| `FR-01.5` | M-01 Autentikasi & Manajemen Akun | `02-modules/m01-auth.md` | In Progress | PR-02-07 (#68): alur utama 1–5, A1, A2 dan AC kode cadangan/peringatan ≤ 2. PR-02-33 (#69): A3 (reset 2FA), A5, A7 (kode aktivasi). PR-02-08 (#78): A6 (CLI penerbit kode aktivasi). TERBUKA: menonaktifkan 2FA sendiri — belum ada PR (logs/phase-02.md §10) |
| `FR-01.6` | M-01 Autentikasi & Manajemen Akun | `02-modules/m01-auth.md` | Done | PR-02-08 (#78): admin:recover (guard admin lain aktif, 2FA dilepas, password+kode aktivasi baru, sesi sistem dicabut, NT-53), diuji |
...
| `FR-04.2` | M-04 Inventaris Aset | `02-modules/m04-assets.md` | In Progress | PR-02-12 (#84): `GET /assets` (langkah 2-4: pencarian kode/nama/merek/nomor seri, filter kategori/lokasi/kondisi/status/tahun/kelayakan pinjam, paginasi `COUNT(*) OVER()`), BR-073 (field finansial) & scope `restricted` (A1) ditegakkan di repository, `GET /rooms/{id}/assets` (FR-03.2) tersambung data sungguhan. TERBUKA: langkah 5 (halaman detail lengkap — riwayat lintas modul), A3 ekspor (belum ada PR) |
| `FR-04.3` | M-04 Inventaris Aset | `02-modules/m04-assets.md` | In Progress | PR-02-13 (#85): `PATCH /assets/{id}/condition` (`asset.update_condition` — bukan `asset.update`, dikonfirmasi pemilik produk; katalog §10 keliru di tabel endpoint §7). Langkah 1-4, alasan wajib (BR-007), status turunan `TIDAK_TERSEDIA` saat `RUSAK_BERAT`/`HILANG` (BR-006), riwayat kondisi (BR-005 dst. tidak tersentuh — kerangka status sudah ada `PR-02-10`). BR-012 (referensi wajib saat `HILANG`) ditegakkan STRUKTURAL (kolom terisi), bukan tervalidasi (M-13/M-21 belum ada). TERBUKA: A1/A2 (pembatalan reservasi + notifikasi) — `booking_slots`/M-17 belum ada, dan `m04-assets.md` §9 menyatakan modul ini tidak menerbitkan notifikasi; scope `ASSIGNED` Teknisi berlaku seperti `all` sementara (`work_orders`/M-12 belum ada) |
| `FR-04.4` | M-04 Inventaris Aset | `02-modules/m04-assets.md` | In Progress | PR-02-14 (#86): `POST /assets/move` (`asset.update`) — langkah 1, 2, 4: 1..50 aset dari ruangan asal mana pun ke SATU ruangan tujuan AKTIF (BR-009) dalam satu transaksi (AC atomik, diuji: satu aset gagal di tengah batch -> tak satu pun berpindah); status wajib `TERSEDIA`/`DALAM_PERBAIKAN` (Preconditions, BR-010, A1); riwayat `asset_movements` (asal, tujuan, tanggal, pelaku, alasan — AC); penanggung jawab baru opsional. TERBUKA: langkah 3 kapasitas lokasi tujuan (tak ada konsep kapasitas aset — `rooms.kapasitas` = orang; ditunda, dikonfirmasi pemilik produk), A1 pemeriksaan pinjaman/reservasi PADA tanggal mutasi (`booking_slots` belum ada — kini hanya `status` saat ini), langkah 5/AC berita acara PDF (pembangkit PDF `SDD-FS-12` belum dibangun), A2 pemindaian QR (klien mobile; API-nya sama) |
| `FR-04.5` | M-04 Inventaris Aset | `02-modules/m04-assets.md` | In Progress | PR-02-15 (belum digabung): `GET/POST /asset-categories`, `PUT/DELETE /asset-categories/{id}` (baris endpoint tulis + A3/A4 dinaikkan ke PRD lebih dulu, keputusan 62). Langkah 1-3, A1 (kode duplikat, buat dan ubah), A2 (kategori terpakai — termasuk aset dihapuskan — tak dapat dihapus, pesan memuat jumlah aset), A3 (induk bersubkategori tak dapat dihapus), A4 (kode kategori terpakai tak dapat diubah), AC dua tingkat (kedalaman tak dibatasi, siklus ditolak). TERBUKA: AC "interval preventif otomatis diusulkan saat membuat jadwal pemeliharaan" — milik FR-12.2/M-12 (Phase 04); kolom `interval_preventif_hari` sudah tersimpan |
