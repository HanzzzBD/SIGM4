...
**Kunci TOTP** (`PR-02-07`). `TOTP_ENCRYPTION_KEY` berformat base64 dari tepat **32 byte** (`openssl rand -base64 32`); ia mengenkripsi secret TOTP (AES-256-GCM, `SDD-SESS-08`) dan **terpisah** dari kunci JWT karena siklus rotasinya berbeda. Startup menolak nilai yang bukan base64 sah atau bukan 32 byte tanpa mencetak isinya. Kehilangan kunci ini berarti seluruh 2FA harus didaftarkan ulang (SDD-04 §6): cadangkan terpisah dari basis data. API memvalidasinya sejak `PR-02-07`; worker menyusul bersama perintah break-glass (`PR-02-08`) yang menghapus 2FA akun.
