...
| **Status** | `Not Started` |
| **Modul PRD** | M-08 Reservasi Barang · M-12 Pemeliharaan · M-13 Stock Opname |
...
Aset memiliki siklus hidup penuh selain peminjaman: dapat dipesan di muka, dirawat, dan diverifikasi keberadaannya. Reservasi barang menutup separuh kedua `booking_slots` — sumber daya kini mencakup ruangan **dan** unit aset dalam satu tabel dan satu constraint.

## 2. Scope

**Termasuk**

- M-08: ketersediaan barang, pengajuan reservasi barang, pembatalan
...
- Serah terima fisik barang (check-out/check-in) → **Phase 05** bersama M-09; reservasi hanya *memesan*, tidak menyerahkan
...
**`M2` tertutup di phase ini** — kriteria keluarnya (50 permintaan simultan atas slot sama → tepat 1 sukses) berlaku bagi ruangan *dan* barang, sehingga baru dapat dinyatakan lulus setelah M-08 hidup.
...
- Kalender ketersediaan barang dengan sumber data yang sama seperti ruangan
- Reservasi barang yang melewati approval dan mengunci slot
...
|---|---|:---:|---|---|---|
| `PR-04-01` | Ketersediaan barang di atas `booking_slots` (`resource_type='asset'`) | M | Ph03 | `FR-08.1`, `SDD-AVL-06` | Satu constraint melayani ruangan & barang |
| `PR-04-02` | Pengajuan reservasi barang + approval | L | 01, Ph02 | `FR-08.2`, `BR-017` … `BR-023c` | Status aset ditetapkan saat aktivasi slot, bukan saat approval |
| `PR-04-03` | Pembatalan reservasi barang + pelepasan slot | M | 02 | `FR-08.3`, `BR-024` `BR-024a` `BR-024b` | Slot `Released`, tidak dihapus |
| `PR-04-04` | Kalender & pencarian ketersediaan barang (UI) | M | 01 | `FR-08.1`, `AV-01` … `AV-05` | Menampilkan alternatif saat penuh |
...
- [ ] Reservasi barang dan ruangan berbagi satu constraint tanpa jalur kode terpisah
...
|---|---|---|---|
| `SlotService` perlu diubah untuk melayani barang | Abstraksi Phase 02 ternyata salah; kerja ulang menyebar | Perubahan `SlotService` di phase ini wajib melalui tinjauan arsitek dan tercatat di log | `SDD-AVL-06` |
...
| Reservasi barang perlu ditarik | Slot bertipe `asset` di-`Released`; slot ruangan tidak tersentuh karena difilter `resource_type` |
...
- [ ] Kriteria keluar `M2` PRD terpenuhi: 50 permintaan simultan atas slot yang sama → tepat 1 sukses, diuji pada ruangan **dan** barang
