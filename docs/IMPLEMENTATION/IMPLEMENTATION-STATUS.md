...
| `PR-02-07` | `Blocked` | — | **Tertahan sebelum push/merge:** penugasan PR pelaksana untuk pengikatan pendaftaran 2FA pertama (teks PRD `BR-070d`/`BR-070e` sudah diputuskan, keputusan 46; kodenya belum ada) dan tinjauan arsitek atas `otp_verified` (opsi A dipertahankan, keputusan 42); konflik gerbang ganti password diselesaikan di PR terpisah (keputusan 43). Merge ke `develop` boleh setelah itu, rilis dilarang sebelum PR reset-2FA (keputusan 45) — analisis di [log phase-02 §7](logs/phase-02.md). Kode dan uji selesai lokal. 2FA TOTP (`FR-01.5`, `BR-070`): login akun ber-2FA berhenti di challenge 5 menit sekali pakai (Redis, buram), `POST /auth/2fa/verify` (TOTP atau kode cadangan, penguncian yang sama dengan password → `423` hanya di langkah ini), pendaftaran `enroll`/`enroll/confirm`, `backup-codes/regenerate`, gerbang `twoFactorVerified` → `403 TWO_FACTOR_REQUIRED` (kode galat baru) pada role wajib 2FA (R-01, R-03) dengan bawaan tertutup dan pengecualian eksplisit; `TOTP_ENCRYPTION_KEY` masuk skema config; migration `0024` (expand). Cabang `feature/PR-02-07-2fa-totp`, belum di-push; **menyentuh lapisan permission/migration → tinjauan arsitek**. `traceability.md` diperbarui hanya untuk `FR-01.5`, `BR-070`, `BR-070c`. Keputusan 33–41 [log phase-02](logs/phase-02.md). Disertai penyegaran status pasca #65/#66/#67 atas instruksi pemilik produk (tanpa PR chore terpisah). Menonaktifkan 2FA sendiri dan reset 2FA oleh Administrator sengaja DI LUAR PR ini (follow-up terpisah, belum ada PR pemilik) — [log phase-02 §10](logs/phase-02.md) |
