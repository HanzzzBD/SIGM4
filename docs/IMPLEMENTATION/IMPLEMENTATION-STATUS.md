...
| [02](phases/phase-02.md) | Inti Sistem | 5 | 31 | `In Progress` | 10/31 | Phase terbesar; di lintasan kritis. Dipensiunkan: `PR-02-01` → `PR-01-16`, `PR-02-09` → `PR-01-15`; `PR-02-31` baru (keputusan 9 log phase-01); `PR-02-33` baru (keputusan 48 log phase-02) |
...
| `PR-02-12` | `In Review` | [#84](https://github.com/HanzzzBD/SIGM4/pull/84) | Pencarian & penyaringan aset + paginasi (`FR-04.2`, `SDD-API-05`, `SDD-PERF-01`): `GET /assets` (`q` lintas kode/nama/merek/nomor seri, filter allow-list, `sort`, `COUNT(*) OVER()`); BR-073 & scope `restricted` lewat SELECT dinamis (`SDD-AUTH-06`); migration `0028` (`pg_trgm` + GIN trigram `nama`/`merek`, expand). `GET /rooms/{id}/assets` (`FR-03.2`) disambungkan ke kueri sungguhan, menutup kerangka `PR-01-07`. Keputusan 59 [log phase-02](logs/phase-02.md) |
| `PR-02-13` … `PR-02-32` | `Not Started` | — | Rincian: [`phases/phase-02.md` §7](phases/phase-02.md) |
