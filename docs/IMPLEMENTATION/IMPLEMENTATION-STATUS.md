...
| `PR-02-07` | `In Review` | [#68](https://github.com/HanzzzBD/SIGM4/pull/68) | PR dibuka 21 September 2026. **Tinjauan `otp_verified` selesai 21 September 2026** (opsi A disetujui pemilik produk; celah balapan diperbaiki, keputusan 49). M2 (`BR-070d` kode aktivasi + endpoint penerbit + reset-2FA) kini `PR-02-33` (keputusan 48); M3 (`BR-070e`, cabut sesi lain) dikerjakan di PR ini (keputusan 46–47). Konflik gerbang ganti password diselesaikan di PR terpisah (keputusan 43). Merge ke `develop` boleh; **rilis dilarang sebelum `PR-02-33` tergabung** (keputusan 45) — analisis di [log phase-02 §7](logs/phase-02.md). 2FA TOTP (`FR-01.5`, `BR-070`): login akun ber-2FA berhenti di challenge 5 menit sekali pakai (Redis, buram), `POST /auth/2fa/verify` (TOTP atau kode cadangan, penguncian yang sama dengan password → `423` hanya di langkah ini), pendaftaran `enroll`/`enroll/confirm`, `backup-codes/regenerate`, gerbang `twoFactorVerified` → `403 TWO_FACTOR_REQUIRED` (kode galat baru) pada role wajib 2FA (R-01, R-03) dengan bawaan tertutup dan pengecualian eksplisit; `TOTP_ENCRYPTION_KEY` masuk skema config; migration `0024` (expand). Cabang `feature/PR-02-07-2fa-totp`; menyentuh lapisan permission/migration — tinjauan arsitek selesai (keputusan 49). `traceability.md` diperbarui hanya untuk `FR-01.5`, `BR-070`, `BR-070c`. Keputusan 33–47 [log phase-02](logs/phase-02.md). Disertai penyegaran status pasca #65/#66/#67 atas instruksi pemilik produk (tanpa PR chore terpisah). Menonaktifkan 2FA sendiri dan reset 2FA oleh Administrator sengaja DI LUAR PR ini (follow-up terpisah, belum ada PR pemilik) — [log phase-02 §10](logs/phase-02.md) |
