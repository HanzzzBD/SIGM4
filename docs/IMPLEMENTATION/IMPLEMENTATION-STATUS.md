...
| `PR-02-06` | `In Progress` | — | Ganti password sendiri + kelola profil (`FR-01.4`): `POST /auth/password/change` (mencabut sesi lain, menerbitkan access token baru berklaim `pwd=false` bagi sesi ini, menyelesaikan penerbitan reset `DITERBITKAN` + `NT-38a`), `GET`/`PUT /me`; foto profil menunggu `PR-03-04`, larangan 3 password terakhir/daftar bocor menunggu `PR-02-31` — keputusan [log phase-02](logs/phase-02.md) |
