...
| [02](phases/phase-02.md) | Inti Sistem | 5 | 31 | `In Progress` | 5/31 | Phase terbesar; di lintasan kritis. Dipensiunkan: `PR-02-01` → `PR-01-16`, `PR-02-09` → `PR-01-15`; `PR-02-31` baru (keputusan 9 log phase-01); `PR-02-33` baru (keputusan 48 log phase-02) |
...
| `PR-02-07` | `Blocked` | — | **Tertahan sebelum push/merge (sejak 20 September 2026):** tanda tangan tinjauan arsitek atas `otp_verified` (telaah disiapkan, celah balapan terbukti empiris; log phase-02 §7). M2 (`BR-070d` kode aktivasi + endpoint penerbit + reset-2FA) kini `PR-02-33` (keputusan 48); M3 (`BR-070e`, cabut sesi lain) sudah dikerjakan di PR ini (keputusan 46–47). Tinjauan arsitek atas `otp_verified` (opsi A dipertahankan, keputusan 42); konflik gerbang ganti password diselesaikan di PR terpisah (keputusan 43). Merge ke `develop` boleh setelah itu, rilis dilarang sebelum PR reset-2FA (keputusan 45) — analisis di [log phase-02 §7](logs/phase-02.md). Kode dan uji selesai lokal. 2FA TOTP (`FR-01.5`, `BR-070`): login akun ber-2FA berhenti di challenge 5 menit sekali pakai (Redis, buram), `POST /auth/2fa/verify` (TOTP atau kode cadangan, penguncian yang sama dengan password → `423` hanya di langkah ini), pendaftaran `enroll`/`enroll/confirm`, `backup-codes/regenerate`, gerbang `twoFactorVerified` → `403 TWO_FACTOR_REQUIRED` (kode galat baru) pada role wajib 2FA (R-01, R-03) dengan bawaan tertutup dan pengecualian eksplisit; `TOTP_ENCRYPTION_KEY` masuk skema config; migration `0024` (expand). Cabang `feature/PR-02-07-2fa-totp`, belum di-push; **menyentuh lapisan permission/migration → tinjauan arsitek**. `traceability.md` diperbarui hanya untuk `FR-01.5`, `BR-070`, `BR-070c`. Keputusan 33–47 [log phase-02](logs/phase-02.md). Disertai penyegaran status pasca #65/#66/#67 atas instruksi pemilik produk (tanpa PR chore terpisah). Menonaktifkan 2FA sendiri dan reset 2FA oleh Administrator sengaja DI LUAR PR ini (follow-up terpisah, belum ada PR pemilik) — [log phase-02 §10](logs/phase-02.md) |
| `PR-02-08` … `PR-02-33` (tanpa `PR-02-09`, pensiun) | `Not Started` | — | Rincian: [`phases/phase-02.md` §7](phases/phase-02.md). `PR-02-33` baru: kode aktivasi 2FA + reset 2FA di M-02 (keputusan 48) — harus tergabung sebelum `PR-02-08` bagian CLI penerbit kode aktivasi |
...
| Rilis (`main`) yang memuat `PR-02-07` | `PR-02-33` (kode aktivasi 2FA + reset 2FA, `FR-01.5 A3`, `POST /users/{id}/reset-2fa`) tergabung (log phase-02 keputusan 44–48). Tanpa reset, pendaftaran oleh pemegang password tak punya jalur pemulihan biasa | 20 September 2026 | Pemilik produk |
