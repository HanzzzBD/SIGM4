...
describe("kanonikal", () => {
    ...
    it("field yang berpindah nilai menghasilkan teks berbeda", () => {
        expect(kanonikal(baris({ modul: "BAHAN" }))).not.toBe(
            kanonikal(baris()),
        );
    });
...
describe("hashBaris (NFR-S-03d)", () => {
    ...
    it("perubahan satu karakter mengubah hash", () => {
        expect(
            hashBaris(
                null,
                baris({ keterangan: "Layar retak saat pengembalian." }),
            ),
        ).not.toEqual(hashBaris(null, baris()));
    });
    ...
    it("setiap field ikut terhitung — tak satu pun terlewat dari hash", () => {
        ...
        for (const field of URUTAN_FIELD) {
            const diubah = hashBaris(
                null,
                baris({ [field]: "NILAI-BERBEDA-UNTUK-UJI" }),
            );
            expect(diubah, `field ${field} tidak ikut terhitung`).not.toEqual(
                dasar,
            );
        }
    });
