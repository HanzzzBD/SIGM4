...
describe("hashRequestBody", () => {
    ...
    it("body identik menghasilkan hash identik", () => {
        expect(hashRequestBody({ room_id: 1 })).toBe(
            hashRequestBody({ room_id: 1 }),
        );
    });
    ...
    it("urutan kunci bersarang juga tidak mengubah hash", () => {
        expect(hashRequestBody({ x: { a: 1, b: 2 } })).toBe(
            hashRequestBody({ x: { b: 2, a: 1 } }),
        );
    });
    ...
    it("nilai berbeda menghasilkan hash berbeda", () => {
        expect(hashRequestBody({ room_id: 1 })).not.toBe(
            hashRequestBody({ room_id: 2 }),
        );
    });
    ...
    it("field undefined diabaikan, field null tidak", () => {
        ...
        expect(hashRequestBody({ a: 1, b: null })).not.toBe(
            hashRequestBody({ a: 1 }),
        );
    });
