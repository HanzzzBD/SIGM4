...
// Direktif @ts-expect-error menempel pada BARIS yang galat, bukan pada awal
// pernyataan: Prettier membungkus pemanggilan ini, dan direktif di atas
// `defineRepository(` menjadi tak terpakai — gerbang kompilasinya diam-diam hilang.
defineRepository({
    // @ts-expect-error — ctx bukan argumen pertama (SDD-AUTH-02)
    findLoans: async (filter: { peminjamId: number }): Promise<Loan[]> => [
        { id: filter.peminjamId },
    ],
});
...
describe("ScopedRepository", () => {
    it("meloloskan repository yang setiap metodenya menerima ctx lebih dulu", async () => {
        await expect(
            repositoriSah.findLoans(ctx, { peminjamId: 7 }),
    ...
    it("meloloskan kelas yang setiap metodenya menerima ctx lebih dulu", async () => {
        ...
        await expect(repo.findLoans(ctx, { peminjamId: 7 })).resolves.toEqual([
            { id: 7 },
        ]);
    });
});
...
describe("BaseRepository.query", () => {
    ...
    it("menolak pemanggil yang menembus tipe lewat cast (kasus penolakan)", async () => {
        ...
        await expect(
            repo.findLoans(undefined as unknown as AuthContext, {
                peminjamId: 7,
            }),
