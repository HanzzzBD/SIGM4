...
const konteks = (
    scopes: ReadonlyArray<readonly [string, Scope]>,
): AuthContext =>
    createAuthContext({
        userId: 7,
        roleCode: "SISWA",
...
describe("assertAuthContext", () => {
    it("meloloskan konteks yang sah", () => {
        expect(() =>
            assertAuthContext(konteks([["loan.view", "own"]])),
        ).not.toThrow();
    });
    ...
    const tidakSah: ReadonlyArray<readonly [string, unknown]> = [
        ...
        [
            "tanpa scopeOf",
            { userId: 1, roleCode: "ADMIN", can: (): boolean => true },
        ],
    ];

    it.each(tidakSah)("menolak %s (SDD-AUTH-02)", (_nama, nilai) => {
        expect(() =>
            assertAuthContext(nilai as unknown as AuthContext),
        ).toThrow(/AuthContext wajib/);
    });
});
