...
function bacaMigration(): ReadonlyMap<string, readonly string[]> {
    ...
    for (const berkas of readdirSync(DIR_MIGRATION)
        .filter((f) => f.endsWith(".sql"))
        .sort()) {
        const teks = readFileSync(new URL(berkas, DIR_MIGRATION), "utf8");
        const naik =
            teks.split("-- migrate:up")[1]?.split("-- migrate:down")[0] ?? "";
        for (const cocok of naik.matchAll(
            /CREATE TYPE\s+(\w+)\s+AS ENUM\s*\(([^)]*)\)/g,
        )) {
...
describe("migration enum terhadap Bab 11.3", () => {
    it("membaca kelompok dari PRD, bukan dari daftar di dalam uji ini", () => {
        ...
        expect(bab113.get("Kondisi Aset")).toEqual([
            "BAIK",
            "RUSAK_RINGAN",
            "RUSAK_BERAT",
            "HILANG",
    ...
    it("nilai tiap tipe identik dengan Bab 11.3, baris per baris", () => {
        ...
        for (const [kelompok, nilai] of bab113) {
            ...
            if (aktual.join(",") !== nilai.join(",")) {
                selisih.push(
                    `${tipe}: PRD=[${nilai.join(",")}] migration=[${aktual.join(",")}]`,
                );
            }
        }
    ...
    it("setiap tipe punya pasangan DROP pada migrate:down (CD-05)", () => {
        ...
        for (const berkas of readdirSync(DIR_MIGRATION).filter((f) =>
            f.endsWith(".sql"),
        )) {
            const turun =
                readFileSync(new URL(berkas, DIR_MIGRATION), "utf8").split(
                    "-- migrate:down",
                )[1] ?? "";
            for (const m of turun.matchAll(/DROP TYPE IF EXISTS (\w+)/g))
                dijatuhkan.add(m[1] ?? "");
        }
...
describe("0001_extensions.sql", () => {
    const teks = readFileSync(
        new URL("apps/api/migrations/0001_extensions.sql", AKAR),
        "utf8",
    );
