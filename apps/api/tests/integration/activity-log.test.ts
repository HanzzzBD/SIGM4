...
describe.skipIf(!ADA_DB)("activity_logs terhadap PostgreSQL nyata", () => {
    ...
    const tulis = (n = 1, ubah: Record<string, unknown> = {}) =>
        withTransaction(
            ...
            async (scope) => {
                for (let i = 0; i < n; i += 1)
                    await audit.write(scope, entri({ ...ubah, entitasId: i }));
            },
            db,
        );
    ...
    it("AL-06 — pekerjaan terjadwal tercatat berpelaku SYSTEM beserta namanya", async () => {
        await withTransaction(
            ctx,
            (scope) =>
                audit.writeSystem(scope.tx, "activity-log-verify", entri()),
            db,
        );
        const b = await kueri<{
            user_id: string | null;
    ...
    it("AL-07 — operasi yang gagal tetap dicatat dengan hasil GAGAL", async () => {
        await withTransaction(
            ...
            (scope) =>
                audit.write(
                    scope,
                    entri({ hasil: "GAGAL", keterangan: "saldo tidak cukup" }),
                ),
            db,
        );
        const b = await kueri<{ hasil: string }>(
            "SELECT hasil FROM activity_logs",
        );
    ...
    it("AL-08 — kegagalan tulis log TIDAK menggagalkan transaksi bisnis", async () => {
        const hasil = await withTransaction(
            ...
            async (scope) => {
                ...
                const tercatat = await audit.write(
                    scope,
                    entri({ modul: null as unknown as string }),
                );
                ...
                const lanjut = await sql<{ n: number }>`SELECT 1 AS n`.execute(
                    scope.tx,
                );
                ...
                await scope.tx
                    ...
                    .onConflict((oc) =>
                        oc.columns(["prefix", "year"]).doNothing(),
                    )
                    .execute();
...
describe.skipIf(!ADA_DB)("AL-03b — hak akun aplikasi", () => {
    ...
    it("boleh INSERT dan SELECT", async () => {
       ...
       VALUES (now(), $1, $2, $3, $4)`,
        ...
        await expect(
            app.query("SELECT count(*) FROM activity_logs"),
    ...
    it("UPDATE langsung ke PARTISI juga ditolak — bukan hanya lewat induknya", async () => {
        ...
        await expect(
            app.query(`UPDATE ${nama} SET modul = 'X'`),
    ...
    it("DDL ditolak seluruhnya (SEC-CFG-03)", async () => {
        await expect(
            app.query("CREATE TABLE coba_ddl (id int)"),
        ).rejects.toMatchObject({
            code: "42501",
        });
    });
