...
import {
    ...
    nomorSah,
} from "../../src/shared/numbering/index.js";
import { dbmate, kueri } from "../helpers/db.js";

const ADA_DB = process.env["DATABASE_URL"] !== undefined;

describe.skipIf(!ADA_DB)(
    "DocumentNumberService terhadap PostgreSQL nyata",
    () => {
        ...
        it("urutan direset per tahun anggaran (SEQ-01, AC-YR-04)", async () => {
            const clock2027 = new FixedClock(
        ...
        it("menolak prefiks di luar SEQ-04 alih-alih menerbitkan nomor tak sah", async () => {
            await expect(
                layanan.next(
                    db,
                    "ABC" as unknown as Parameters<typeof layanan.next>[1],
                ),
        ...
        it("1.000 permintaan PARALEL menghasilkan 1.000 nomor UNIK dan berurutan tanpa lompatan", async () => {
            const JUMLAH = 1000;
        ...
        it("paralel lintas prefiks tidak saling mengganggu", async () => {
            ...
            const hasil = await Promise.all(
                prefiks.flatMap((p) =>
                    Array.from({ length: 50 }, () => layanan.next(db, p)),
                ),
            );
