...
describe.skipIf(!ADA_DB)("PR-01-14 — gerbang persetujuan wali (acceptance)", () => {
    ...
    describe("lewat HTTP", () => {
        ...
        async function kirim(ctx: AuthContext, body: unknown): Promise<{ status: number; json: unknown }> {
            ...
            app.use(
                "/api/v1",
                usersRouter({ db: getDb(), penerbitPassword: penerbitPalsu, pengelolaDuaFaktor: pengelolaPalsu, auditLogger: audit(), logger: logger(), clock: new FixedClock(T1) }, batasi, authorize),
            );
            app.use(
                ujungRantai({
