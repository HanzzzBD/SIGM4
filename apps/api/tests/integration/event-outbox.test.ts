...
describe.skipIf(!ADA_DB)("Outbox terhadap PostgreSQL nyata", () => {
    ...
    function dispatcherPencatat(handlers?: EventHandlerRegistry) {
        ...
        const registry =
            handlers ??
            new EventHandlerRegistry().on(
                "LoanReturned",
                async (e: OutboxEvent) => {
                    jejak.push(
                        `${e.aggregateId}:${String((e.payload as { urutan?: number }).urutan)}`,
                    );
                },
            );
        ...
        Number(
            (
                await kueri<{ n: string }>(
                    `SELECT count(*)::text AS n FROM event_outbox ${where}`,
                )
            )[0]?.n,
        );
    ...
    it("SDD-EVT-09 — DUA dispatcher bersamaan TIDAK membalik urutan satu agregat", async () => {
        const jejak: string[] = [];
    ...
    it("kegagalan yang berulang berakhir sebagai DEAD LETTER dan berhenti dipungut", async () => {
        ...
        await withTransaction(
            ctx,
            (scope) => publish(scope, acara("LoanReturned", 5)),
            db,
        );
    ...
    it("backoff menahan agregat sampai waktunya, tanpa memblokir agregat lain", async () => {
        ...
        await withTransaction(
            ...
            (scope) =>
                publishAll(scope, [
                    acara("LoanReturned", 11),
                    acara("LoanReturned", 12),
                ]),
            db,
        );
    ...
    it("event tanpa handler ditandai selesai, bukan dianggap gagal", async () => {
        ...
        await withTransaction(
            ctx,
            (scope) => publish(scope, acara("BelumAdaKonsumennya", 1)),
            db,
        );
    ...
    it("menyimpan actor_id dan request_id agar jejak melewati batas commit", async () => {
        await withTransaction(
            ctx,
            (scope) => publish(scope, acara("LoanReturned", 1, { a: 1 })),
            db,
        );
    ...
    it("processed_at diambil dari Clock yang di-inject (SDD-SYS-07)", async () => {
        ...
        await withTransaction(
            ctx,
            (scope) => publish(scope, acara("LoanReturned", 1)),
            db,
        );
        await dispatcher.drain();
        ...
        expect(baris[0]?.processed_at.toISOString()).toBe(
