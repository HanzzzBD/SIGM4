...
describe.skipIf(!ADA_REDIS)("Worker skeleton terhadap Redis nyata", () => {
    ...
    it("DUA instance worker → job dieksekusi TEPAT SEKALI (JOB-02)", async () => {
        ...
        pekerja.push(
            createWorker(redisBaru(), registry),
            createWorker(redisBaru(), registry),
        );
    ...
    it("penjadwalan dari DUA instance tidak menghasilkan dua jadwal (JOB-02)", async () => {
        const registry = new JobRegistry().register(
            {
                name: "probe-jadwal",
                cron: "5 17 * * *",
        ...
        expect(penjadwal.map((p) => p.key).sort()).toEqual([
            "probe-jadwal",
            "probe-jadwal-2",
        ]);
    }, 30_000);

    it("jadwal tersimpan dalam UTC (JOB-04)", async () => {
        const registry = new JobRegistry().register({
            name: "probe-utc",
            cron: "5 17 * * *",
            handler: async () => undefined,
    ...
    it("job yang gagal dicoba ulang, tidak ditelan diam-diam (JOB-06)", async () => {
        let percobaan = 0;
        const registry = new JobRegistry().register({
            name: "probe-gagal",
            cron: "*/1 * * * *",
        ...
        const gagal: string[] = [];
        pekerja.push(
            createWorker(redisBaru(), registry, (nama) => gagal.push(nama)),
        );
    ...
    it("job tanpa definisi GAGAL KERAS, bukan ditelan", async () => {
...
describe("JobRegistry — tanpa Redis", () => {
    it("menolak nama yang didaftarkan dua kali", () => {
        ...
        expect(() => new JobRegistry().register(j, j)).toThrow(
            JobRegistrationError,
        );
    });
