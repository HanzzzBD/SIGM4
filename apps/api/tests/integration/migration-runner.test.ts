...
describe.skipIf(process.env["DATABASE_URL"] === undefined)(
    ...
    () => {
        ...
        afterAll(async () => {
            ...
            for (let sisa = await probeTerpasang(); sisa > 0; sisa -= 1)
                dbmate("down");
            for (const nama of SEMUA_PROBE)
                rmSync(join(DIR_MIGRATION, nama), { force: true });
            dbmate("up");
        });
        ...
        it("membuka transaksi secara bawaan — CREATE INDEX CONCURRENTLY ditolak 25001", () => {
            ...
            try {
                ...
                pesan = String(
                    (galat as { stderr?: string; message?: string }).stderr ??
                        galat,
                );
            }
            expect(pesan).toMatch(
                /25001|cannot run inside a transaction block/i,
            );
        });
