...
function asWib(d: Date): string {
    return new Date(d.getTime() + 7 * 60 * 60_000)
        .toISOString()
        .replace(".000Z", "")
        .replace("T", " ");
}

describe.skipIf(!ADA_DB)(
    "BusinessCalendarService terhadap PostgreSQL nyata",
    () => {
    ...
    `);
        ...
        describe("isWorkingDay", () => {
            it("Senin adalah hari kerja", async () => {
                expect(
                    await kalender.isWorkingDay(db, wib("2026-09-07T09:00:00")),
                ).toBe(true);
            });

            it("Minggu bukan hari kerja — `work_days` menonaktifkannya", async () => {
                expect(
                    await kalender.isWorkingDay(db, wib("2026-09-13T09:00:00")),
                ).toBe(false);
            });
            ...
            it("hari libur bukan hari kerja meski jatuh pada hari aktif", async () => {
                ...
                expect(
                    await kalender.isWorkingDay(db, wib("2026-09-08T09:00:00")),
                ).toBe(false);
            });
        ...
        describe("addWorkingHours — CAL-01", () => {
            it("menambah di dalam hari yang sama", async () => {
                const hasil = await kalender.addWorkingHours(
                    db,
                    wib("2026-09-07T08:00:00"),
                    3,
                );
      ...
      `);
            ...
            it("menolak jumlah jam negatif", async () => {
                await expect(
                    kalender.addWorkingHours(
                        db,
                        wib("2026-09-07T09:00:00"),
                        -1,
                    ),
                ).rejects.toThrow(/non-negatif/);
            });

            it("gagal terang-terangan bila tidak ada satu pun hari kerja", async () => {
                await kueri("UPDATE work_days SET aktif = false");
                await expect(
                    kalender.addWorkingHours(db, wib("2026-09-07T09:00:00"), 1),
                ).rejects.toThrow(/work_days/);
            });
        });
        ...
        describe("workingMinutesBetween — CAL-01", () => {
            ...
            it("nol bila urutan terbalik atau sama", async () => {
                expect(
                    await kalender.workingMinutesBetween(
                        db,
            ...
            it("kebalikan addWorkingHours pada rentang yang sama", async () => {
                ...
                expect(
                    await kalender.workingMinutesBetween(db, mulai, tenggat),
                ).toBe(5 * 60);
            });
        });
