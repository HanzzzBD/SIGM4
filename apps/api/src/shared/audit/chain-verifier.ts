...
export async function ensurePartitions(
    ...
    for (let i = 0; i <= bulanKeDepan; i += 1) {
        const dari = new Date(
            Date.UTC(awal.getUTCFullYear(), awal.getUTCMonth() + i, 1),
        );
        const sampai = new Date(
            Date.UTC(awal.getUTCFullYear(), awal.getUTCMonth() + i + 1, 1),
        );
    ...
    `.execute(db);
        await sql`REVOKE UPDATE, DELETE, TRUNCATE ON ${sql.ref(nama)} FROM sigm4_app`.execute(
            db,
        );
        dibuat.push(nama);
    }
