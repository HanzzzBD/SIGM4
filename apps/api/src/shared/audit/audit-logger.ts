...
export class AuditLogger {
    ...
    private readonly onFailure:
        ((entry: AuditEntry, galat: unknown) => void) | undefined;

    constructor(opsi: OpsiAuditLogger) {
        this.clock = opsi.clock;
        this.logger =
            opsi.logger ??
            new Logger({ clock: opsi.clock, modulBawaan: "audit" });
    ...
    private async simpan(
        ...
        try {
            ...
            await sql`SELECT pg_advisory_xact_lock(${KUNCI_RANTAI})`.execute(
                tx,
            );
      ...
      `.execute(tx);
            ...
            const baris: BarisKanonik = {
                ...
                nilai_sebelum:
                    entry.nilaiSebelum === undefined
                        ? null
                        : redact(entry.nilaiSebelum),
                nilai_sesudah:
                    entry.nilaiSesudah === undefined
                        ? null
                        : redact(entry.nilaiSesudah),
            ...
            await tx
                ...
                .values({
                    ...
                    user_id:
                        pelaku.userId === null ? null : String(pelaku.userId),
                    ...
                    entitas_id:
                        entry.entitasId === undefined
                            ? null
                            : String(entry.entitasId),
            ...
            await sql`ROLLBACK TO SAVEPOINT audit_log`
                .execute(tx)
                .catch(() => undefined);
