...
export class BusinessCalendarService {
    ...
    private isWorkingWibDay(wib: Date, calendar: CalendarSnapshot): boolean {
        return (
            ...
            !calendar.holidays.has(wibDateKey(wib))
        );
    }
    ...
    async isWorkingDay(
        ...
        return this.isWorkingWibDay(
            wib,
            await this.loadCalendar(executor, key, key),
        );
    }
    ...
    async addWorkingHours(
        ...
        if (!Number.isFinite(hours) || hours < 0) {
            throw new Error(
                "Jumlah jam kerja harus bilangan non-negatif (CAL-01).",
            );
        }
        ...
        const calendar = await this.loadCalendar(
            ...
            wibDateKey(
                new Date(
                    cursor.getTime() +
                        MAX_DAYS_SCANNED * DAY_MINUTES * MINUTE_MS,
                ),
            ),
        );
        ...
        for (let scanned = 0; scanned <= MAX_DAYS_SCANNED; scanned += 1) {
            ...
            if (remaining <= available) {
                return fromWibClock(
                    new Date(
                        dayStart.getTime() + (from + remaining) * MINUTE_MS,
                    ),
                );
            }
    ...
    async countCalendarDays(
        ...
        const total = Math.round(
            (endDay.getTime() - startDay.getTime()) / (DAY_MINUTES * MINUTE_MS),
        );
        ...
        for (let i = 1; i <= total; i += 1) {
            const day = new Date(
                startDay.getTime() + i * DAY_MINUTES * MINUTE_MS,
            );
