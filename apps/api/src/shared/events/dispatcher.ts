...
export class OutboxDispatcher {
    ...
    private readonly onDeadLetter:
        ((event: OutboxEvent, galat: unknown) => void) | undefined;
    ...
    constructor(options: DispatcherOptions) {
        ...
        this.logger =
            options.logger ??
            new Logger({ clock: options.clock, modulBawaan: "outbox" });
    ...
    async tick(): Promise<TickResult> {
        ...
        return this.db.transaction().execute(async (tx) => {
            ...
            for (const baris of antrean) {
                ...
                try {
                    for (const handler of this.registry.handlersFor(
                        event.name,
                    )) {
                        await handler(event);
                    }
            ...
            this.backoff.delete(
                kunciBackoff(dipilih.aggregate_type, dipilih.aggregate_id),
            );
